...
const MakeTheoryMaterialPage = () => {
  ...
  useEffect(() => {
    async function initText() {
      const rec = await getTheoryText(theoryId, false);
      
      setContent(rec.text);
      setTitle(rec.name);
    }
    initText();
  }, [theoryId, setContent, setTitle]);
  ...
  const validateLink = () => {
    ...
    if (newLink.link && !isValidHttpUrl(newLink.link))
      newErrors.link = "Введите валидный адрес ссылки";
  ...
  const handleAddDocument = async () => {
    ...
    const document = await createTheoryDoc(
      theoryId,
      newDoc.file,
      newDoc.description
    );
  ...
  const handleAddLink = async () => {
    ...
    const link = await createTheoryLink(
      theoryId,
      newLink.link,
      newLink.description
    );
  ...
  const handleUpdateTitle = async () => {
    const result = await updateTheoryTitle(theoryId, title);
    if (!result) {
      setSaveMessage({
        isTitleError: true,
        titleMessage: "Ошибка при обновлении названия лекции",
      });
    } else {
      setSaveMessage({
        isTitleError: false,
        titleMessage: "Лекция успешно обновлена",
      });
    }
    ...
    <Layout paths={paths}>
      <Card className="shadow-lg">
        <Card.Header className="bg-light">
          {saveMessage.titleMessage && (
            <Alert
              variant={saveMessage.isTitleError ? "danger" : "success"}
              className="mt-3"
            >
              {saveMessage.titleMessage}
            </Alert>
